...
- コードは **CodeProject Open License (CPOL) ライセンスの条件に従って再配布** しています。  
...
現状、ロジック部分はほぼオリジナルのままですが、現代の環境でビルド・利用できるように主に以下の修正を行なっています。
...
- 型サイズ差による警告の修正（キャストで暫定的に）  
- x64 で問題なく動作するよう最小限のコード調整

### ✔ その他の軽微な変更
- コメントの追加  
- ビルドエラーの解消  
- Unicode 対応の準備（必要があれば）
